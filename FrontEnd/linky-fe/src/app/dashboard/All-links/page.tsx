...
export default function Dashboard() {
    ...
    const getDeviceIcon = (device: string) => {
        switch (device.toLowerCase()) {
            case 'mobile': return <IconDeviceMobile className="h-5 w-5" />;
            case 'desktop': return <IconDeviceDesktop className="h-5 w-5" />;
            case 'tablet': return <IconDeviceTablet className="h-5 w-5" />;
            default: return <IconDeviceDesktop className="h-5 w-5" />;
        }
    };
    ...
    const fetchDashboardData = async () => {
        ...
        try {
            ...
            if (response.status === 200) {
                const { links, totalLinks, totalVisits, totalRedirects, peakVisitTime, topDevices } = response.data.data;
                setLinks(links);
                setStats({
                    totalLinks,
                    totalVisits,
    ...
    if (loading) {
        return (
            <div className="flex-col h-screen w-full gap-4 flex items-center justify-center">
                <div className="w-16 h-16 sm:w-20 sm:h-20 border-4 border-transparent text-blue-700 animate-spin flex items-center justify-center border-t-blue-700 rounded-full">
                    <div className="w-12 h-12 sm:w-16 sm:h-16 border-4 border-transparent text-[#c890f9] animate-spin flex items-center justify-center border-t-[#c890f9] rounded-full"></div>
                </div>
            </div>
        );
    }
